...
export const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-white/5 backdrop-blur-xl border-b border-white/10 shadow-lg transition-all duration-700 ease-in-out ${
      ...
      <div className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className={`flex items-center space-x-3 transition-all duration-700 ease-out ${
            ...
            <img style={{ width: "120px", height: "auto" }} src="/logo.jpeg" alt="" />
          ...
          <div className="lg:hidden">
            ...
            >
              <span className={`w-6 h-0.5 bg-white transform transition-all duration-300 ease-in-out ${
                isMobileMenuOpen ? 'rotate-45 translate-y-1.5' : ''
              }`}></span>
              <span className={`w-6 h-0.5 bg-white transition-all duration-300 ease-in-out ${
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navigationItems.map((item, index) => (
              <div key={item.label} className={`transition-all duration-700 ease-out ${
        ...
        {/* Mobile Menu Overlay */}
        {isMobileMenuOpen && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 lg:hidden" onClick={toggleMobileMenu}></div>
        )}

        {/* Mobile Menu */}
        <div className={`fixed top-0 right-0 h-full w-80 max-w-[90vw] bg-white/10 backdrop-blur-xl border-l border-white/20 z-50 transform transition-transform duration-300 ease-in-out lg:hidden ${
